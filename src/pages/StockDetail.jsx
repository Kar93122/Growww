...
export default function StockDetail() {
  ...
  useEffect(() => {
    ...
    const intervalSeconds = timeframe === '1D' ? 300 : timeframe === '1W' ? 3600 : 86400;
    const interval = setInterval(() => {
      setCandles(prev => {
        if (prev.length === 0) return prev;
        const last = prev[prev.length - 1];
        const next = generateNextTick(symbol, last, intervalSeconds);
  ...
  }, [symbol, timeframe, candles.length > 0]);
    ...
    if (candles.length > 0) {
      activeIndicators.forEach(id => {
        ...
        if (Array.isArray(result)) {
          vals[id] = result[result.length - 1];
        } else if (result && typeof result === 'object') {
          const keys = Object.keys(result);
          vals[id] = result[keys[0]]?.[result[keys[0]].length - 1];
        }
      });
    }
    return vals;
